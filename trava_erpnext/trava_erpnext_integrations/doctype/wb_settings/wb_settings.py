...
from trava_erpnext.trava_erpnext_integrations.doctype.wb_settings.wb_report_methods import get_report
...
class WBSettings(Document):
	...
	def get_products_details(self):
		if self.enable_wb == 1:
			frappe.enqueue('trava_erpnext.trava_erpnext_integrations.doctype.wb_settings.wb_methods.get_products_details')

	def get_order_details(self):
		if self.enable_wb == 1:
			after_date = dateutil.parser.parse(self.after_date).strftime("%Y-%m-%d")
			frappe.enqueue('trava_erpnext.trava_erpnext_integrations.doctype.wb_settings.wb_methods.get_orders', after_date=after_date)
